...
void RenderScene(void)
	...
	glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);//加深度

	// Turn culling on if flag is set
	if(iCull)
		glEnable(GL_CULL_FACE);//显示可透明
	else
	...
	if(iOutline)
		glPolygonMode(GL_BACK,GL_LINE);//显示线
	else
	...
	glBegin(GL_TRIANGLE_FAN);//扇形
	//顶点=Pinnacle
	// Pinnacle of cone is shared vertex for fan, moved up Z axis
	// to produce a cone instead of a circle
	glVertex3f(0.0f, 0.0f, 75.0f);//第一个点是扇形的中心点,在Z轴正方向应该是指向自己,这是指向显示器里面???
	...
	glVertex2f(0.0f, 0.0f);//第一个点是扇形的中心点
	for(angle = 0.0f; angle < (2.0f*GL_PI); angle += (GL_PI/8.0f))
		{
		// Calculate x and y position of the next vertex
		x = 50.0f*sin(angle);
...
void SetupRC()
	...
	glShadeModel(GL_FLAT);//平的

	// Clock wise wound polygons are front facing, this is reversed
	// because we are using triangle fans
	glFrontFace(GL_CW);//的逆时针绕法是正面对着,逆时针 Counter ClockWise
	}
