...
namespace c11_wchar
{
	...
	void readChineseFile()
	{
		...
		wint_t wc;//wint_t类型
		int n = 0;
		pFile=fopen ("c:/tmp/c_file.txt","r");//文件名不支持中文 ????????
		if (pFile!=NULL)
		{
			do
			{
	...
	void wchar_number()
	{
		//----------wchar_t 与　数　转换
		wchar_t result[20];
		swprintf(result,L"%ld",-123.33);//使用swprintf函数,在vc2019中 项目->属性->C/C++->预处理器(preprocessor)->预处理器定义(preprocessor Definitions)中添加 _CRT_NON_CONFORMING_SWPRINTFS
	...
	void wchar_method()
	{
		...
		wprintf (L"source: %ls \n",cn);//CDT不显示????,VC控制台显示???,VC debug变量正常
		wprintf (L"target: %ls \n",buffer);//CDT不显示????,VC控制台显示???,VC debug变量正常
	...
	void wchar_char()
	{
		...
		char  cn[30];
		strcpy(cn,"中华人民共和国");
		int len=mbrlen(cn,30,&state);//1 错误???  mb=multibyte
		printf("len=%d \n",len);//
